        ...
        foreach ($this->translatedComponents as $dqlAlias => $comp) {
            ...
            foreach ($config['fields'] as $field) {
                ...
                if ($ea->usesPersonalTranslation($transClass)) {
                    ...
                    /** @author pcabreus Esto permite usar la clase heredada para hacer la consulta y no la clase hija*/
                    $config = $this->listener->getConfiguration($em, $meta->name);
                    $name = is_array($config) && $config['useObjectClass'] ? $config['useObjectClass'] : $meta->name;

                    $sql .= ' AND '.$tblAlias.'.'. $transMeta->getQuotedColumnName('objectClass', $this->platform)
                        .' = '.$this->conn->quote($name);
